...
class HomogeneousHiddenMarkovModel:
    """
    Homogeneous Hidden Markov Model with discrete emissions.
    """
    def __init__(self, n_states=2, n_emits=2, max_iter=1000, tolerance=1e-5, init_seed=None):
        """
        ...
        self.param_history = None               # List of parameter histories for multiple starts
    ...
    def init_param(self, init_A=None, init_B=None):
        """
    ...
    def fit(self, observations, init_A=None, init_B=None, n_starts=1, max_iter:int=None, tolerance:float=None, verbose=False):
        """
        ...
        param_history = []      # List of parameter histories for n_starts, every element is a list of parameters for each iteration
        ...
        for j in range(n_starts):
            self.init_param(init_A=init_A, init_B=init_B)
            n_iters, loglik, loglik_list, param_list = self.run_em(observations, max_iter=max_iter, tolerance=tolerance, verbose=verbose)
    ...
    def run_em(self, observations, max_iter=None, tolerance=None, verbose=False):
        """
        ...
        for i in range(self.max_iter):
            ...
            new_param = self._get_param()
            delta_param = np.abs(old_param - new_param).max() # can also check the np.linalg.norm
    ...
    def _update_model(self, param):
        ...
        '''
        self.A[:] = [[1-param[0], param[0]], [param[1], 1-param[1]]]
        self.B[:] = [[1-param[2], param[2]], [param[3], 1-param[3]]]
