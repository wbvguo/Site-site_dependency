...
class HeterogeneousMarkovChain:
    ...
    def _neglog_likelihood(self, observations):
        """
    ...
    def fit(self, observations, init_prob=None, init_w=None, n_starts=1, verbose=False, verbose_result=False):
        """
        ...
        def objective(trans_param):
            ...
            loss = self._neglog_likelihood(observations)
            if verbose: # not a local variable, access from outer scope
                loss_list.append(loss)
                param_list.append(trans_param)
            return loss
        ...
        for i in tqdm(range(n_starts), disable=not verbose):
            ...
            param_list= [] # clean before collect parameters
